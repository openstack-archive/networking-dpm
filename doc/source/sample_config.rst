...
But certain ML2 configuration options in the ML2 configuration file
(typically *ml2_conf.ini*) are required in order to use it.

Enable the dpm mechanism driver
+++++++++++++++++++++++++++++++

The *dpm* mechanism driver must be enabled using the *mechanism_drivers*
config option. Typically the DPM mechanism driver must be configured amongst
other mechanisms (like *ovs*) that are used on the network node or
compute nodes managing other hypervisor types (like KVM). The following example
enables the *ovs* and the *dpm* mechanism driver in Neutrons ml2 config file
*ml2_conf.ini*::
...
corresponding vlan ranges must be defined in ML2 configuration file::
...
.. note::
  This config option is not required for VLAN provider networks.
