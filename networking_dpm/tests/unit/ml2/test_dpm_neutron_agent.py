...
class TestDPMMain(base.BaseTestCase):
    ...
    def test__get_physnet_adapter_map(self):
        ...
        actual = dpm_agt._get_physnet_adapter_map(cpc)
    ...
    def test__get_physnet_adapter_map_adapter_not_found(self):
        ...
        with mock.patch.object(sys, 'exit') as m_sys:
            dpm_agt._get_physnet_adapter_map(cpc)
            m_sys.assert_called_with(1)

    def test__get_physnet_adapter_map_empty(self):
        mapping = {}
        cfg.CONF.set_override('physical_adapter_mappings', mapping,
                              group='dpm')
        with mock.patch.object(sys, 'exit') as m_sys:
            dpm_agt._get_physnet_adapter_map(mock.Mock())
            m_sys.assert_called_with(1)

    def test__get_physnet_adapter_map_multiple_adapters(self):
