...
class DPMMechanismDriver(mech_agent.SimpleAgentMechanismDriverBase):
    ...
    def try_to_bind_segment_for_agent(self, context, segment, agent):
        ...
        vswitch_ids = self.get_mappings(agent)[physnet]
        if not vswitch_ids:
            LOG.warning(_LW("No vswitch_ids found in agents %(agent)s mapping "
                            "for physical network %(net)s."), {'agent': agent,
                                                               'net': physnet})
            return False
        if len(vswitch_ids) > 1:
            LOG.warning(_LW("More than one vswitch_id for physical network "
                            "%(net)s on agent %(agent)s found but only a "
                            "single vswitch_id is supported. Therefore the "
