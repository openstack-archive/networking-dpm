...
class DPMManager(amb.CommonAgentManagerBase):
    ...
    @staticmethod
    def _is_openstack_managed(nic):
        # TODO(andreas_s): There is no way for the agent to determine if
        # the uuid is actually in use by Neutron, or if it is just a valid
        # uuid. Probably some other identifier might be usefull, e.g. in the
        # NICs 'description' field
        return uuidutils.is_uuid_like(nic.get_property('name'))
    ...
    def get_all_devices(self):
        ...
        for physnet_vswitches in self.physnet_adapter_map.values():
            for vswitch_id in physnet_vswitches:
                ...
                for nic in nics:
                    # Nova store the Neutron Port UUID in the NICs name field
                    if self._is_openstack_managed(nic):
...
def _get_physnet_adapter_map(cpc):
...
def main():
    ...
    physnet_adapter_map = _get_physnet_adapter_map(cpc)

    manager = DPMManager(physnet_adapter_map, cpc)
