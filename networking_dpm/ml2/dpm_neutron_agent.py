...
class PhysicalNetworkMapping(object):
    ...
    * the polling against the HMC for new NICs
    ...
    def _add_vswitch(self, physnet, vswitch):
    ...
    def _create_mapping(self):
        interface_mappings = self._get_interface_mapping_conf()
        ...
        for physnet, adapter_port_dict in interface_mappings.items():
            ...
            for adapter_uuid, port in adapter_port_dict.items():
                ...
                adapter = self._get_adapter(adapter_uuid)
                self._validate_adapter_type(adapter)
                self._validate_adapter_port(adapter, port)
                vswitch = self._get_vswitch(adapter_uuid, port)
                self._add_vswitch(physnet, vswitch)
...
def main():
    ...
    physnet_vswitch_map = PhysicalNetworkMapping(cpc)
