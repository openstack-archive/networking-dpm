...
#auth_url='http://192.168.50.4:5000/v3'
...
# Cleaning up precreated partitions
...
# Create the vnic and set the neutron port uuid as port name like nova would do it 
nic = partition.nics.create({"virtual-switch-uri": vswitch_uri, "name": port_id})
...
assert("dpm" == get_neutron_port(port_id)['binding:vif_type'])
print "Neutron port vif type set to 'dpm'"
assert(vswitch_id == get_neutron_port(port_id)['binding:vif_details']['vswitch_id'])
